...
while cap.isOpened():
    ...
    frame = cv2.resize(frame, (1600, 1200))  # Adjust dimensions for a larger frame
    ...
    try:
        ...
        if knee_angle > 160:  # Standing position
            stage = "up"
        if knee_angle < 90 and stage == "up":  # Squat position
            stage = "down"
            counter += 1
        ...
        cv2.putText(image, str(int(knee_angle)),
                    tuple(np.multiply(knee, [1600, 1200]).astype(int)),  # Adjust for resized frame
    ...
    cv2.putText(image, 'SQUAT COUNT', (15, 30),
